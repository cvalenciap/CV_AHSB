...
function ob_t2_unchk_parent(e) {
	...
	if (e!=null){
		if ((e.firstChild!=null) && (typeof(e.firstChild.tagName) != 'undefined' && e.firstChild.tagName == 'INPUT') && (e.firstChild.id.substr(0,4)=="chk_")) {
...
function ob_hasChildren(ob_od)
{
	try
	{
		return (ob_od.parentNode.parentNode.parentNode.parentNode.tagName.toLowerCase() == 'div' && (ob_od.parentNode.parentNode.parentNode.parentNode.className.toLowerCase() == 'ob_t2b' || ob_od.parentNode.parentNode.parentNode.parentNode.className.toLowerCase() == 'ob_t2c') && ob_od.parentNode.parentNode.parentNode.parentNode.childNodes.length > 1);
	}
	catch (e)
	{
		return false;
...
function ob_getFurthestChildOfNode (ob_od, expand)
{
	...
	while (true)
	{
		...
		{
			if (!ob_isExpanded(fChild) && expand)
			{
				try
				{
					fChild.parentNode.firstChild.firstChild.onclick();
				} catch (e) {}
			}
...
function ob_t2_Add(parentId, childId, textOrHTML, expanded, image, subTreeURL)
{
	...
	if (!ob_hasChildren(pNode))
	{
		...
		// set it to minus (the parent node will be expanded)
		e.src=ob_style+"/minus_l.gif";
		// attach the expand/collapse function
		...
		// if the parent of parent has other children, we need to add a vertical bar
	...
	tree_node_exp_col = true;
	document.getElementById(parentId).onclick();
	tree_node_exp_col = false;
...
function ob_saveNodeState( objBlockNode, NodeId ) {
	...
	if( objBlockNode.style.display == "block" ) {
		ob_setCookie(NodeId,"false",ExpireDate);
	}
	else {
		ob_setCookie(NodeId,"true",ExpireDate);
	}
}
...
function ob_nodeEnsureVisible(ob_od)
{
	...
	if (tree.offsetWidth == 0 || tree.offsetHeight == 0) return;
	if (!tree.style.pixelWidth) tree.style.pixelWidth = eval(tree.offsetWidth);
	if (!tree.style.pixelHeight) tree.style.pixelHeight = eval(tree.offsetHeight);
	
	var av_width = tree.clientWidth;
	var av_height = tree.clientHeight;
	
	var work_ob_od = ob_od.parentNode.parentNode.parentNode;
	
	topPos = (ob_getTop(work_ob_od) - ob_getTop(tree) - tree.scrollTop);
	bottomPos = topPos + work_ob_od.offsetHeight;
	leftPos = (ob_getLeft(work_ob_od) - ob_getLeft(tree) - tree.scrollLeft);
	rightPos = leftPos + work_ob_od.offsetWidth;

	dif = bottomPos - av_height + 1;
	if (topPos < 0) tree.scrollTop += topPos;
	else if (dif > 0) tree.scrollTop += dif;
	
	dif = rightPos - av_width + 1;
